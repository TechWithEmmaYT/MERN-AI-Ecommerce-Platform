...
const AccountAddressesPage = () => {
  ...
  const [addresses, setAddresses] = useState<AddressType[]>(starterAddresses);
  ...
  const createAddressMutation = {
    isPending: false,
    mutate: (values: AddressFormValues) => {
      const nextAddress: AddressType = {
        _id: `starter-address-${Date.now()}`,
        userId: "starter-user",
