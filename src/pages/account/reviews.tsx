...
import { createReviewMutationFn } from "@/lib/api";
import { toast } from "sonner";
import { Link } from "react-router-dom";
import {
  reviewableItems as starterReviewableItems,
  submittedReviews as starterSubmittedReviews,
...
const reviewableList: ReviewableEntry[] = starterReviewableItems.map((item) => ({
  item: {
    _id: item.id,
...
const submittedList: SubmittedEntry[] = starterSubmittedReviews.map((review) => ({
  _id: review.id,
  productId: {
      ...
      <CardContent className="grid gap-4 p-4 md:grid-cols-[76px_1fr]">
        ...
        <div className="flex min-w-0 flex-col gap-3">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            >
              {createReviewMutation.isPending && <span className="size-4 rounded-full border-2 border-current border-r-transparent animate-spin" />}
              Submit review
            </Button>
          </div>
        </div>
